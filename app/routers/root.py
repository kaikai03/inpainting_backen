...
async def test(*, count: int):
    ...
    # return test_
    t = video_item(cover='http://127.0.0.1:90/imgs/1.jpg', src='http://127.0.0.1:90/v/1.mp4')
    t2 = video_item(index=1, cover='http://127.0.0.1:90/imgs/2.jpg', src='http://127.0.0.1:90/v/2.mp4')
    t3 = video_item(index=2, cover='http://127.0.0.1:90/imgs/3.gif', src='http://127.0.0.1:90/v/3.mp4')
    t.dict()
    t.dict(exclude_none=True)
    t.json(exclude_defaults=True)
    video_item(**dict({"cover": "http://127.0.0.1:90/imgs/1.jpg", "src": "http://127.0.0.1:90/v/1.mp4"}))
    return [t, t2, t3]

@router.get("/dbtest/")
async def init_db():
    con.global_db.completed.insert()
    return "ok"
