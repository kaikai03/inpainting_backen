...
class auto_time_table(table.Table):
    ...
    def auto_increate_item_id(self) -> int:
        ...
        # 主要用于视频工作项的全局生成序号
        db.table(__base_table__).update(increment('item_id_auto'), where('item_id_auto').exists())
        return db.table(__base_table__).get(where('item_id_auto').exists())['item_id_auto']
...
db.table_class = auto_time_table
base_tb = db.table(__base_table__)
if len(base_tb) == 0:
    base_tb.insert({"item_id_auto": 0})
workqueue_tb = db.table(__workqueue_table__)
history_tb = db.table(__history_table__)
trash_tb = db.table(__trash_table__)
